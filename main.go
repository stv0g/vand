...
// ssd1351 writes to a display driven by a ssd1351 controler.
package main
...
import (
	...
	"periph.io/x/devices/v3/ssd1351"
...
func findFile(name string) string {
	...
	for _, p := range strings.Split(os.Getenv("GOPATH"), ":") {
		if len(p) != 0 {
			if p2 := filepath.Join(p, "src/periph.io/x/cmd/ssd1351", name); access(p2) {
				return p2
			}
		}
	}
...
func mainImpl() error {
	spiID := flag.String("spi", "/dev/spidev0.0", "SPI port to use")
	dcName := flag.String("dc", "25", "DC pin to use in 4-wire SPI mode")
	var hz physic.Frequency
	flag.Var(&hz, "hz", "SPI port speed")

	h := flag.Int("h", 128, "display height")
	...
	var s *ssd1351.Dev
	opts := ssd1351.Opts{W: *w, H: *h, Rotated: *rotated, Sequential: *sequential, SwapTopBottom: *swapTopBottom}
		...
		s, err = ssd1351.NewSPI(c, dc, &opts)
		if err != nil {
			return err
		}
...
func main() {
	if err := mainImpl(); err != nil {
		fmt.Fprintf(os.Stderr, "ssd1351: %s.\n", err)
		os.Exit(1)
	}
}